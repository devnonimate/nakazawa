...
const CampaignGroupFieldsConfig = ({ selectedEmpresa }) => {
  ...
  const campaignFields = [
    { id: 'id', description: 'ID do grupo de campanha.' },
    { id: 'account_id', description: 'ID da conta de anúncios associada ao grupo de campanhas.' },
    { id: 'objective', description: 'Objetivo da campanha (exemplo: TRAFFIC, AWARENESS, SALES).' },
    { id: 'name', description: 'Nome do grupo de campanhas.' },
    { id: 'configured_status', description: 'Status configurado do grupo de campanha (exemplo: ACTIVE, PAUSED).' },
    { id: 'effective_status', description: 'Status efetivo da campanha, considerando qualquer condição ou limitação.' },
    { id: 'buying_type', description: 'Tipo de compra (exemplo: AUCTION, FIXED_PRICE).' },
    { id: 'created_time', description: 'Data e hora de criação do grupo de campanhas.' },
    { id: 'updated_time', description: 'Data e hora da última atualização do grupo de campanhas.' },
    { id: 'spend_cap', description: 'Limite de gasto do grupo de campanhas.' },
    { id: 'can_use_spend_cap', description: 'Indica se é possível usar um limite de gasto para o grupo de campanhas.' },
    { id: 'issues_info', description: 'Informações sobre problemas ou restrições aplicadas ao grupo de campanhas.' },
    { id: 'special_ad_categories', description: 'Categorias especiais de anúncios, se aplicáveis (exemplo: POLITICAL, SOCIAL_ISSUES).' }
  ];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const url = `${config.API_URL}/api/campaign-groupfields?campaigngroupfields=${selectedCampaign}&fields=${selectedFields.join(',')}&email=${encodeURIComponent(storedEmail)}&nome_empresa=${encodeURIComponent(selectedEmpresa)}&anuncio=${encodeURIComponent(selectedCampaign)}`;
      const response = await fetch(url, { headers: { Accept: 'application/json', 'ngrok-skip-browser-warning': 'true' } });
...
export default CampaignGroupFieldsConfig;
